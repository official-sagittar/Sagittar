...
namespace sagittar {
    ...
    void Engine::setPositionFromFEN(std::string fen) { core::fen::parseFEN(&board, fen); }
    ...
    search::types::SearchResult Engine::search(
      search::types::SearchInfo                               info,
    ...
    void Engine::bench() {
        std::array<std::string, 50> positions = {
        ...
        for (const auto& fen : positions)
        {
            ...
            searcher.reset();
            const search::types::SearchResult result = search(info);
