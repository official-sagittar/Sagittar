    ...
    namespace search {
        ...
        namespace tt {
            ...
            void TranspositionTable::store(const u64              hash,
                ...
                if (value < -constants::MATE_SCORE)
                {
                    value -= ply;
                }
                else if (value > constants::MATE_SCORE)
                {
                    value += ply;
                }
                ...
                core::move::Move move_to_replace = move;
                if ((move == core::move::Move()) && (currentry.key == hash))
                {
