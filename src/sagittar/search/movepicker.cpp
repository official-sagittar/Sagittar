        ...
        namespace movepicker {
            ...
            void scoreMoves(commons::containers::ArrayList<core::move::Move>* moves,
                ...
                for (u8 i = 0; i < moves->size(); i++)
                {
                    ...
                    else if (core::move::isCapture(move.getFlag()))
                    {
                        const PieceType attacker = pieceTypeOf(board.getPiece(move.getFrom()));
                        const PieceType victim =
                          (move.getFlag() == core::move::MoveFlag::MOVE_CAPTURE_EP)
                            ? PieceType::PAWN
            ...
            void sortMoves(commons::containers::ArrayList<core::move::Move>* moves,
                           const u8                                          index) {
