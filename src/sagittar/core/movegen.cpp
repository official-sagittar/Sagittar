...
namespace sagittar {
    ...
    namespace core {
        ...
        namespace movegen {
            ...
            getVariant(const u32 index, const u8 bits, board::BitBoard mask) {
                i32             i, j;
            ...
            static u32
            transform(const board::BitBoard b, const board::BitBoard magic, const u8 bits) {
            ...
            static u64 findMagic(const Square sq, const u8 bits, const bool bishop) {
                ...
                for (i32 i = 0; i < (1 << n); i++)
                {
                    ...
                    attacks[i] =
                      bishop ? bishopAttacks(sq, variants[i]) : rookAttacks(sq, variants[i]);
                }
            ...
            static void initMagicTableBishop() {
                for (u8 sq = Square::A1; sq <= Square::H8; sq++)
                {
                    ...
                    MAGICTABLE_BISHOP[sq].magic =
                      findMagic(square, MAGICTABLE_BISHOP[sq].shift, true);
#ifdef DEBUG
                ...
                for (u8 sq = 0; sq < 64; sq++)
                {
                    ...
                    for (i32 i = 0; i < (1 << n); i++)
                    {
                        ...
                        ATTACK_TABLE_ROOK[sq][magic_index] =
                          rookAttacks(static_cast<Square>(sq), b);
                    }
                }
            }
            ...
            static board::BitBoard getKnightAttacks(const Square          sq,
                                                    const board::BitBoard occupancy) {
            ...
            static board::BitBoard getKingAttacks(const Square          sq,
                                                  const board::BitBoard occupancy) {
                return ATTACK_TABLE_KING[sq] & occupancy;
            }

            static void
            generatePseudolegalMovesPawn(commons::containers::ArrayList<move::Move>* moves,
            ...
            static void
            generatePseudolegalMovesPiece(commons::containers::ArrayList<move::Move>* moves,
                                          const board::Board&                         board,
                                          const MovegenType                           type) {
                const Color     active_color = board.getActiveColor();
            ...
            static void
            generatePseudolegalMovesCastle(commons::containers::ArrayList<move::Move>* moves,
                                           const board::Board&                         board) {
