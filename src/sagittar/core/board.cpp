    ...
    namespace core {
        ...
        namespace board {
            ...
            [[nodiscard]] DoMoveResult Board::doMove(const move::Move move) noexcept {
                ...
                movePiece(piece, from, to, move::isCapture(flag), move::isPromotion(flag),
                          promoted);
            ...
            [[nodiscard]] DoMoveResult Board::doMove(const std::string& move_str) noexcept {
                ...
                else if (pieceTypeOf(pieces[from]) == PieceType::KING)
                {
                    if (active_color == Color::WHITE)
                    {
                        ...
                        {
                            flag = is_capture ? move::MoveFlag::MOVE_CAPTURE
            ...
            void Board::undoMove() {
                ...
                else
                {
                    ...
                    piece                = move::isPromotion(flag)
                                           ? pieceCreate(PieceType::PAWN, prev_active_color)
                                           : piece;
