...
namespace sagittar {
    ...
    void Position::setPinned() {
        BitBoard pinned = 0ULL;

        const Square   king_sq_us = m_king_sq;
        const BitBoard our_pieces = m_bb_colors[m_stm];
        const BitBoard op_pieces  = m_bb_colors[colorFlip(m_stm)];

        const BitBoard queens = m_bb_pieces[PieceType::QUEEN];
    ...
    [[nodiscard]] bool Position::isLegalMove(const Move& move) const noexcept {
        // Movegen generates pseudolegal moves considering:
        //  1. Pinned pieces can only move along the direction of the pin
        //  2. When in check, generate only check evasions and blockers
        //  3. When in check and multiple checkers exist, only generate King moves
        //  4. For casteling, approproate casteling rights and a clear path exists

        // TODO: Add checks for moves not generated by movegen

        const Square from   = move.from();
        const Square to     = move.to();
        const Piece  move_p = m_board[from];

        // Moving piece should be of the right color
        if (pieceColorOf(move_p) != m_stm)
        {
            return false;
        }

        // King should not move to a square that lands it in check
        if (pieceTypeOf(move_p) == PieceType::KING)
        {
            const BitBoard kingless_occ = occupied() ^ BB(m_king_sq);
            return (squareAttackers(*this, to, colorFlip(m_stm), kingless_occ) == 0ULL);
        }

        if constexpr (!Generated)
        {
            if (utils::bitCount1s(m_checkers) > 1)
            {
                return false;
            }
        }

        // En Passant capture should not leave our king in discovered check
        if (move.flag() == MoveFlag::MOVE_CAPTURE_EP)
        {
            const i8       dir          = (m_stm == Color::WHITE) ? 8 : -8;
            const Square   ep_victim_sq = static_cast<Square>(to - dir);
            const BitBoard post_ep_occ  = occupied() ^ BB(from) ^ BB(to) ^ BB(ep_victim_sq);
            ...
            return ((attacks<PieceType::BISHOP>(m_king_sq, post_ep_occ) & op_bq) == 0ULL)
    ...
    template bool Position::isLegalMove<true>(const Move& move) const noexcept;
    template bool Position::isLegalMove<false>(const Move& move) const noexcept;

    template<Color US, MoveFlag F>
    void Position::applyMove(const Move& move) noexcept {
        ...
        const BitBoard king_bb_them = m_bb_pieces[PieceType::KING] & m_bb_colors[them];
    ...
    void Position::doMove(const Move& move) noexcept {
        if (m_stm == Color::WHITE)
        {
            (this->*apply_move_dispatch_table<Color::WHITE>[move.flag()])(move);
        }
        else
        {
            (this->*apply_move_dispatch_table<Color::BLACK>[move.flag()])(move);
        }
    }
    ...
    [[nodiscard]] bool Position::doMove(const std::string& move_str) noexcept {
        ...
        const bool is_legal_move = isLegalMove<false>(move);
        if (is_legal_move)
        {
            doMove(move);
        }
